...
class base
{
	...
	protected function parse_args(): void
	{
		...
		$this->root_dir = ( $this->opts['d'] ?? $this->opts['dir'] ?? getcwd() ) . DIRECTORY_SEPARATOR;
		$this->cake_dir = $this->root_dir . ".cake" . DIRECTORY_SEPARATOR;
		$this->cake_id = (string) Ulid::generate( TRUE );

		$this->bakes_dir = $this->cake_dir . "bakes" . DIRECTORY_SEPARATOR;
		$this->layers_dir = $this->cake_dir . "layers" . DIRECTORY_SEPARATOR;
		$this->recipes_dir = $this->cake_dir . "recipes" . DIRECTORY_SEPARATOR;
	}
	...
	protected function copy_recursive( string $source, string $dest, array $ignore_dirs = [ '.cake', '.git' ] ): bool
	{
		if ( !is_dir( $source ) )
		{
		...
		foreach ( $iterator as $file )
		{
			...
			{
				if( str_contains( $sub_path, $ignore ) )
				{
					$continue = TRUE;
					break;
				}
